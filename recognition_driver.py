...
def test_lfw_plot():
    ...
    for NUM_FEATURES in range(25,26,5):
        labels, images = FileReader.readFilesToVectors('./lfw-deepfunneled', NUM_FEATURES)
        ...
        for label in set(labels):
            if label == "Venus_Williams":
                pred = f.predict('./lfw-deepfunneled/' + label + '/' + label + '_0001.jpg', 'jpeg')
...
def test_yale(n):
    images, labels = yalefaces.yale_data(n)
    f = Fisherfaces(images, labels, n)
